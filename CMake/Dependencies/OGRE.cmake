...
macro( findPluginAndSetPath BUILD_TYPE CFG_VARIABLE LIBRARY_NAME )
	...
	if( WIN32 )
                if( EXISTS "${OGRE_BINARIES}/bin/${BUILD_TYPE}/${REAL_LIB_PATH}.dll" )
                    set( REAL_LIB_PATH "${OGRE_BINARIES}/bin/${BUILD_TYPE}/${REAL_LIB_PATH}.dll" )
                else()
                    set( REAL_LIB_PATH "${OGRE_BINARIES}/bin/${REAL_LIB_PATH}.dll" )
                endif()
	else()
...
if( NOT IOS )
        ...
        #find_package( SDL2 PATHS "${SDL2_BINARIES}")
        #if( NOT SDL2_FOUND )
        #        message( "Could not find SDL2. https://www.libsdl.org/" )
               ...
               set( OGRE_DEPENDENCY_LIBS ${OGRE_DEPENDENCY_LIBS} ${SDL2_LIBRARIES} )
        #endif()
endif()
...
macro( addStaticDependencies OGRE_SOURCE, OGRE_BINARIES, OGRE_BUILD_SETTINGS_STR, OGRE_DEPENDENCY_LIBS )
	if( WIN32 )
        ...
        if( WIN32 )
                set( OGRE_DEPENDENCIES "${OGRE_DEPENDENCIES_DIR}/lib/$(ConfigurationName)" CACHE STRING
                         "Path to OGRE's dependencies folder. Only used in Static Builds" )
        elseif( IOS )
                set( OGRE_DEPENDENCIES "${OGRE_DEPENDENCIES_DIR}/lib/$(CONFIGURATION)" CACHE STRING
                         "Path to OGRE's dependencies folder. Only used in Static Builds" )
        else()
                set( OGRE_DEPENDENCIES "${OGRE_DEPENDENCIES_DIR}/lib" CACHE STRING
                         "Path to OGRE's dependencies folder. Only used in Static Builds" )
        endif()
